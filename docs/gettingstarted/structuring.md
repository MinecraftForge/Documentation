...
|                          version |       String       |       ""       | The version of the mod. It should be just numbers seperated by dots. Even if `useMetadata` is set to `true`, it's a good idea to put the version here anyways. |
...
| clientSideOnly<br>serverSideOnly | boolean<br>boolean | false<br>false | If either is set to `true`, the mod will only run on the respective side. If both are true, the game crashes. |
...
|                 canBeDeactivated |       boolean      |      false     | If the mod can be deactivated (e.g. a minimap mod), set this to `true` and [recieve](../events/intro.md#creating-an-event-handler) `FMLDeactivationEvent` to perform cleanup tasks. |
|                       guiFactory |       String       |       ""       | Path to the mod's gui factory, if one exists. It must implement `IModGuiFactory`. |
