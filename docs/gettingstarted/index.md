...
3. Make sure Gradle's task finished properly. If not, consult its output to fix errors (common issues are firewall and the `JAVA_HOME` environment variable not pointing to a valid JDK);
4. NetBeans requres the _Gradle Support_ plugin, available from NB's _Plugin Portal_ (http://plugins.netbeans.org/plugin/44510/gradle-support). Install it and restart NB if needed;
5. Inside NB, choose _File > Open Project_ and select the Forge MDK/SRC extraction you made earlier. If everything is OK, it'll appear with Gradle's circle icon;
6. Right click on the Forge project inside the _Projects_ tab. Select _Tasks > run > runClient_.
