...
In the case of adding an inventory to an custom ItemStack of your mod, requires the `Item` class to override `initCapabilities` and return a custom class implementing `ICapabilityProvider` that overrides the above methods for exposing capabilities.
